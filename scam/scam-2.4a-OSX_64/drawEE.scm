...
(define nopolarity 'nopolarity)

(define short 'short)
(define shortstart 'shortstart)
(define shortend 'shortend)
(define none nil)
...
(define ohms "$\\Omega$")
(define ohm ohms)
(define kiloohms "k\\mkern2mu\\Omega$")
(define kiloohm kiloohms)
(define megaohms "M$\\mkern2mu\\Omega$")
(define megaohm megaohms)
(define milliohms "m$\\mkern2mu\\Omega$")
(define milliohm milliohms)
(define microohms "$\\mu\\mkern2mu\\Omega$")
...
(define (resistor direction value units @)
    (if (not (or (member? short @) (member? shortstart @))) (wire direction 1))
    ...
    (cond ((or (and (member? polarity @) (member? reverse @))
          ...
          ((member? polarity @)
            (addLabel resistorLabelOffset (flipDirection direction) minusSign)))
    (apply shift (resistorPolarityAdjust direction 'start))
    (define finishx currentX)
    (define finishy currentY)
    (if (not (or (member? short @) (member? shortend @))) (wire direction 1))
    (define lastx currentX)
    (define lasty currentY)
...
(define (loop-current name @)
    (label name)
    (cond
        ((member? ccw @)
            (arc 0.5 0 270)
            (shift 0 -0.5)
            (wire right 0.2 arrow)
            )
        (else
            (arc 0.5 -90 180)
            (shift 0 -0.5)
